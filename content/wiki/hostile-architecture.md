---
title: "Hostile architecture"
date: 2024-11-10
draft: false
math: false
---

{{< toc >}}

## What is it?

One bright day, we were traveling along EDSA---Epifanio de los Santos
Avenue, one of the main highways in Metro Manila, along which, buildings
of various shapes and sizes stand---when I saw a closed-down building
with its few steps to the facade occupied with rolls of barbed wire.
This is **hostile architecture**, designed and installed---sometimes ad
hoc, sometimes included as part of its original design---to deter
homeless people from squatting the space. Not to mention, it also makes
any space uglier and less welcoming. Now we can say that we should not
put all faith in [design](/design).

Hostile architecture in general not only targets homeless people, but
also anyone, to limit their behavior in whatever way in public spaces.
This is what happens when state agents and [capitalists](/capitalism)
conscript designers who are more than willing to impose hostility to
others to protect private properties from "freeloaders".

Go to any mall, and notice the lack of readily available benches and
chairs for tired people. The floor is not the ideal resting place for
many of us, especially people with limited mobility. And even if one
could sit on floor, it is prohibited in some places because it is
"loitering".

## Real world examples

![A metal bench with a handle in the middle; back rests face opposite directions on each half of bench](/image/hostile-archi-01.jpg)

Found somewhere in Bonifacio Global City, Taguig City, on 14 MAY 2025

![Two concrete benches with no backrest, side by side and facing opposite directions. In each bench, there are two separate depressions, allowing only a two maximum comfortable seats](/image/hostile-archi-02.jpg)

Found around Sunken Garden in UP Diliman, Quezon City, on 23 MAY 2025

![Concrete circular table surrounded four conrete benches. The seat of each bench is like the top half of a cylinder when a plane cuts it, connecting the diameters of opposite circle ends](/image/hostile-archi-03.jpg)

Found inside Quezon Memorial Circle, Quezon City, on 28 NOV 2025

![A steel pipe constructed horizontally to function as a...bench?](/image/hostile-archi-04.jpg)

Found in a waiting shed in Barangay Santolan, Pasig City, on 08 DEC
2025
